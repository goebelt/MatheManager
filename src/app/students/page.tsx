...
import { User, Plus, Search, Edit2, Trash2, Clock, Users as UsersIcon, Calendar, X, UserCheck, UserX } from 'lucide-react';
...
export default function StudentsPage() {
  ...
  const handleUpdateStudent = () => {
    ...
    updatedData.lastUpdated = new Date().toISOString();
    saveData(updatedData);
    setEditingStudentId(null); resetForm();
        ...
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <User className="w-6 h-6 text-green-600" /> Schülerverwaltung
              </h1>
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                Verwalte alle Schüler und deren Einstellungen
              </p>
            </div>
            <button onClick={() => { resetForm(); setShowAddForm(true); setEditingStudentId(null); }}
              className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors"
            >
              <Plus size={18} /> Neuer Schüler
            </button>
          </div>
          ...
          <div className="mt-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Nach Schüler oder Familie suchen..." value={searchTerm}
          ...
          <div className="flex gap-4 mt-4">
            ...
            <div className="px-3 py-1.5 bg-gray-100 dark:bg-slate-700 rounded-lg text-sm">
              ...
              <span className="text-gray-500 dark:text-slate-400">Aktiv</span>
            </div>
            {inactiveCount > 0 && (
              <div className="px-3 py-1.5 bg-gray-100 dark:bg-slate-700 rounded-lg text-sm">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {showAddForm && (
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-sm p-6 mb-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Neuen Schüler anlegen</h2>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Vorname *</label>
                <input type="text" value={formFirstName} onChange={e => setFormFirstName(e.target.value)} placeholder="Max"
              ...
              <div>
                ...
                <input type="text" value={formLastName} onChange={e => setFormLastName(e.target.value)} placeholder="Müller"
                  className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Familie *</label>
                <select value={formFamilyId} onChange={e => setFormFamilyId(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500"
                >
              ...
              <div>
                ...
                <select value={formRhythm} onChange={e => setFormRhythm(e.target.value as 'weekly' | 'biweekly')}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500"
                >
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={formNotes} onChange={e => setFormNotes(e.target.value)} placeholder="Zusätzliche Informationen..." rows={2}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500"
                />
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
              ...
              {formPreferredSchedule.length > 0 ? (
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400">Keine bevorzugten Termine angegeben</p>
              )}
            </div>

            <div className="flex justify-end gap-2 mt-6">
              <button onClick={() => { setShowAddForm(false); setEditingStudentId(null); resetForm(); }}
              ...
              <button onClick={handleAddStudent} disabled={!formFirstName.trim() || !formFamilyId}
        ...
        {filteredStudents.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Noch keine Schüler angelegt</h2>
            <p className="text-gray-500 dark:text-slate-400 mb-6">Erstelle deinen ersten Schüler um loszulegen.</p>
            <button onClick={() => { resetForm(); setShowAddForm(true); setEditingStudentId(null); }}
              className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors"
            >
              <Plus size={18} /> Ersten Schüler anlegen
            </button>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredStudents.map(student => {
              ...
              return (
                <div key={student.id} className={`bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden ${isInactive ? 'opacity-50' : ''}`}>
                  {isEditing ? (
                    /* Edit Form */
                    <div className="p-4 space-y-4">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Schüler bearbeiten</h3>
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Vorname *</label>
                      ...
                      <div className="flex justify-end gap-2 mt-6">
                        <button onClick={() => { setEditingStudentId(null); resetForm(); }}
                          className="px-4 py-2 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
                        >
                          Abbrechen
                        </button>
                        <button onClick={handleUpdateStudent} disabled={!formFirstName.trim() || !formFamilyId}
                          className="px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          Speichern
                        </button>
                      ...
                      <div className="px-4 py-3 bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-medium ${
                            ...
                            {isInactive ? <UserX size={20} /> : <User size={20} />}
                          </div>
                          <div>
                            <h3 className={`font-semibold ${isInactive ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-900 dark:text-white'}`}>
